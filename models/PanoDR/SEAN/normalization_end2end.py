        ...
        if self.use_rgb:
            ...
            if self.status == 'UI_mode':
                ...
                if self.cond:
                    ...
                    component_mu = _middle_mu.reshape(self.style_length, self.in_channels, 1).expand(self.style_length, self.in_channels, component_mask_area)
                    alphas_codes_component = segmap
                    middle_avg = component_mu * alphas_codes_component.squeeze(0).reshape(3,-1)
                    ...
                    for i in range(b_size):
                        for j in range(segmap.shape[1]):
                            #component_mask_area = torch.sum(segmap.bool()[i, j]) #Not used now.
                            component_mask_area = segmap.shape[2]*segmap.shape[3]
                            ...
                            if component_mask_area > 0:
                                ...
                                alphas_codes_component = segmap[i,j].unsqueeze(0)
                                middle_avg[i].masked_scatter_(segmap.bool()[i, j], component_mu) 
                                middle_avg[i] = middle_avg[i] * alphas_codes_component
