...
class ACE(nn.Module):
    def __init__(self, config_text, norm_nc, label_nc, device, ACE_Name=None, status='train', spade_params=None, use_rgb=True, use_soft_sean=False):
        super().__init__()
                ...
                for i in range(b_size):
                    for j in range(segmap.shape[1]):
                        ...
                        if component_mask_area > 0:
                            ...
                            if self.use_soft_sean:
                                component_mask_area = segmap.shape[2] * segmap.shape[3]
                                _middle_mu = F.relu(self.__getattr__('fc_mu1')(style_codes))
                                component_mu = _middle_mu.reshape(self.style_length, self.in_channels, segmap.shape[0]).unsqueeze(3).expand(self.style_length, self.in_channels, segmap.shape[0], component_mask_area)
                                component_mu = component_mu.permute(2,1,0,3)
                                #Component_mu = [512,3,B,H*W]
                                alphas_codes_component = segmap
                                alphas_codes_component = alphas_codes_component.reshape(segmap.shape[0],segmap.shape[1],-1).unsqueeze(2)
                                middle_avg = component_mu * alphas_codes_component
                                middle_avg = middle_avg.sum(dim=1)
                                middle_avg = middle_avg.unsqueeze(0).reshape(segmap.shape[0],self.style_length,segmap.shape[2], segmap.shape[3])
                            else:
